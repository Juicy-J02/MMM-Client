...
func _ready():
	...
	rows = ceili(float(cards.size()) / COLLECTION_WIDTH)
	draw_collection(COLLECTION_WIDTH, rows, cards)
...
func sort(method):
	var names = CardDatabase.CARDS.keys().filter(func(card_name):
		return card_name in Global.collection
	)
