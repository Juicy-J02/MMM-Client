...
func _ready():
	$"../../../CardManager".update_deck.connect(update_collection_visuals)
	...
	for i in range(rows):
		for j in range(columns):
			if card_index < cards.size():
				...
				new_card.position = Vector2((j * 325) + 200, (i * 250) + 200)
				...
				card_index += 1
	container.custom_minimum_size = Vector2(columns * 325, rows * 250 + 200)
